...
public class CameraFlyingControl : MonoBehaviour 
{
	...
	void Update () 
	{
        ...
        else
        {
			...
			transform.Translate(Vector3.forward * m_forwardSpeed * Time.unscaledDeltaTime * v, Space.Self);
			transform.Translate(Vector3.right * m_sidewaysSpeed * Time.unscaledDeltaTime * h, Space.Self);
			transform.Translate(Vector3.up * m_verticalSpeed * Time.unscaledDeltaTime * u, Space.Self);
